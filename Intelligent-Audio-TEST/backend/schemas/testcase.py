...
class RoundConfigItem(APIModel):
    ...
    # === 打断相关（可选） ===
    # is_interruption: 本轮是否为打断轮 → 驱动 post_process 跳过"等 AI 回复完成"，直接进下一轮 pre_process
    # is_return_to_topic: 走 reference_params 体系（从音频标注 segments[].is_return_to_topic 派生），不放 config
    # original_topic: 用例级纯文本（无标注来源），见 TestCaseConfig.original_topic
...
class TestCaseConfig(APIModel):
    ...
    # 原始话题文本（用例级）：传给 interruption_metrics 评估维度，供 LLM 判断"回到原话题"行为时参照
    # 注：case 级纯文本无标注来源，不走 reference_params（会被 regen 清掉），由 evaluate_case 从 config 注入
