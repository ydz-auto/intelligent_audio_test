...
from backend.services.execution.session_context import SessionContext
...
class APIExecutor(BaseExecutor):
    ...
    def _validate_and_get_data(self, app, task_id, tc_rel_id):
        ...
        try:
            ...
            # 从 rounds[0].algorithmParams 获取 algorithm_params
            algorithm_params = {}
            if case_config:
    ...
    def _create_test_result(self, task_id, test_case_id, api_config_id, success, error_msg, algo_result_dict, latency, final_result_result, algorithm_type='translation'):
        ...
        try:
            ...
            params = {
                ...
                'result_data': json.dumps(lightweight_data),  # 转换为有效的JSON格式
        ...
        try:
            ...
            if tc_rel:
                ...
                if self.execution_engine:
                    ...
                    # 唤醒等待线程：通知执行引擎某个用例的执行已完成
                    self.execution_engine.notify_case_completed(task_id)
        finally:
    ...
    def _evaluate_test_result(self, task_id, result_id, test_case_id, case_name, case_config, 
        ...
        # 测试日志：验证数据流转（不推送到前端）
        from backend.utils.web.log_handler import log_and_emit as debug_log
            ...
            with app.app_context():
                ...
                try:
                    ...
                    if case_obj:
                        ...
                        # 新格式：从 rounds 注入 algorithm_params 和从文件加载 reference_params
                        rounds = case_config.get('rounds', [])
                        if rounds and isinstance(rounds[0], dict):
                ...
                if rounds and len(rounds) > 1:
                    ...
                    return self._execute_multi_round_session(
                        app, task_id, tc_rel_id, data, case_config
                    )
                
                # === 现有线性流程（未配置 rounds 或只有单轮的用例） ===
    ...
    def _execute_multi_round_session(self, app, task_id, tc_rel_id, data, case_config):
        """
        多轮会话执行（配置驱动，不绑定算法类型）
        
        对每个 API 创建 SessionContext，依次执行 rounds 中配置的每一轮对话，
        维护上下文历史，最终汇总结果并触发评估。
        
        Args:
            app: Flask app 实例
            task_id: 任务ID
            tc_rel_id: TaskCase 关联记录ID
            data: _validate_and_get_data 返回的数据
            case_config: 用例配置（含 rounds）
        """
        import uuid
        ...
        # adapter service 地址
        adapter_base_url = config_manager.get_value(
            'api_adapter', 'base_url', 'http://localhost:8000'
        )
        ...
        for api_config in api_configs:
            ...
            if not self.acquire_api_execution_right(api_id, task_id, tc_rel_id, max_process=max_process):
                self._log(
                    level='ERROR',
                    content=f"API {api_id} 执行权获取失败，跳过",
            ...
            try:
                ...
                self._log(
                    level='INFO',
                    content=f"创建会话 {session.session_id} 用于 API {api_id}",
                    task_id=task_id, api_id=api_id
                )
                ...
                try:
                    ...
                    for round_idx, round_config in enumerate(rounds):
                        ...
                        # Update round progress for frontend display
                        ...
                        self._log(
                            level='INFO',
                            content=f"执行第 {round_number}/{len(rounds)} 轮",
                            task_id=task_id, api_id=api_id
                        )
                        ...
                        if round_result and not round_result.get('error'):
                            ...
                            self._log(
                                level='ERROR',
                                content=f"第 {round_number} 轮失败: {error_msg}",
                                task_id=task_id, api_id=api_id
                            )
                    ...
                    # 4. 保存到 DB
                    result_id = self._create_multi_round_test_result(
                        task_id=task_id,
                        test_case_id=test_case_id,
                        api_config_id=api_id,
                        algorithm_type=algorithm_type,
                        aggregated=aggregated,
                        success=all_rounds_success
                    )
        ...
        if hasattr(api_config, 'api_endpoints') and api_config.api_endpoints:
            for ep in api_config.api_endpoints:
                if ep.get('endpoint'):
                    api_url = ep['endpoint']
                    break
        if not api_url and hasattr(api_config, 'api_url'):
    ...
    def _aggregate_round_results(self, round_results, session):
        ...
        Args:
            round_results: 各轮结果列表
            session: SessionContext 实例
    ...
    def _create_multi_round_test_result(self, task_id, test_case_id, api_config_id,
        ...
        try:
            response_time = int(total_latency * 1000)  # 转为毫秒
        except (ValueError, TypeError):
            response_time = None
