...
class E2ECollector:
    ...
    def collect_results(self, task_id, test_case_id, device_info_list, **kwargs):
        ...
        self._log(
            ...
            content=f"[e2e_collector] before return: all_results id={id(all_results)}, "
                    f"raw_keys[0]={list(all_results[0].get('raw_results', {}).keys())[:10] if all_results else 'empty'}",
            task_id=task_id
        )
        ...
        self._log(
            ...
            content=f"[e2e_collector] returning: result_to_return id={id(result_to_return)}, "
                    f"raw_keys[0]={list(_first_device.get('raw_results', {}).keys())[:10] if _first_device else 'empty'}",
            task_id=task_id
        )
    ...
    def _calculate_actual_offset(self, task_id, playback_timestamps):
        ...
        for play_time in audio_play_times:
            ...
            self._log(
                level='DEBUG',
                content=f"时间戳分析: audio_id={audio_id}, play_order={play_order}, "
                        f"record_start={record_start_time:.3f}, actual_play={actual_time:.3f}, "
                        f"theory_offset={theory_offset:.3f}, actual_offset={actual_offset:.3f}",
                task_id=task_id
            )
