...
def _convert_device_output_to_algorithm_result(algorithm_type: str, device_output: dict, device_driver_type: str):
    ...
    try:
        ...
        mapped_results = algorithm_query_acl_repository.convert_device_output(algorithm_type, device_output)
...
def _calculate_adjusted_reference_params(extracted_result, original_reference_params, task_id=None, test_case_id=None, device_id=None, playback_time_offsets=None, algorithm_type=None):
    ...
    try:
        ...
        from device_service.domain.services.timestamp_aligner import TimestampAligner
        from device_service.infrastructure.acl.algorithm_query_acl_repository import algorithm_query_acl_repository
        aligner = TimestampAligner(field_query_service=algorithm_query_acl_repository)
...
class DeviceResultReextractor:
    ...
    def _filter_reextractable_devices(self, task_id):
        """筛选支持重新提取的设备"""
        from device_service.infrastructure.drivers.device_driver import device_driver_factory
        ...
        # 通过 DeviceRepository 查询 device_service 自有的 Device PO
        device_ids = [td.get('device_id') for td in task_devices if td.get('device_id')]
        if not device_ids:
            return {}
        devices = device_repository.get_devices_by_ids(device_ids)
    ...
    def _query_task_cases(self, task_id, execution_status, evaluation_status):
        ...
        filtered = []
        for tc in tc_list:
            if tc.get('execution_status') != execution_status:
                continue
            if evaluation_status and tc.get('evaluation_status') != evaluation_status:
                continue
            filtered.append(tc)
        return filtered
    ...
    def _reextract_single_case(self, task_id, tc_rel, device_map):
        ...
        original_reference_params = self._get_original_reference_params(tc_detail, task_id, test_case_id)
        ...
        for device_id, device in device_map.items():
            ...
            algorithm_type = (device_results[0].get('algorithm_type') if device_results
                              else (tc_detail.get('algorithm_type') if tc_detail and tc_detail.get('algorithm_type') else 'asr'))
            ...
            old_ids = [r.get('id') for r in existing_result_map.get(device_id, [])]
    ...
    def _build_existing_result_map(self, existing_results):
        ...
        for result in existing_results:
            ...
            if did:
                result_map.setdefault(did, []).append(result)
        return result_map
    ...
    def _get_original_reference_params(self, test_case, task_id, test_case_id):
        """获取原始参考参数"""
        if not test_case or not test_case.get('config'):
            return None
        from device_service.infrastructure.acl.algorithm_query_acl_repository import algorithm_query_acl_repository
        from shared.utils.dto_utils import dto_to_dict
        ref_col = test_case.get('reference_params')
        if ref_col:
            params = algorithm_query_acl_repository.get_all_reference_params(ref_col)
        else:
            params = algorithm_query_acl_repository.get_all_reference_params(test_case.get('config'))
        # ACL 仓储返回 DTO，下游（TimestampAligner 等）期望 dict，转换回 dict
        params = [dto_to_dict(p) for p in params] if params else None
        if params:
