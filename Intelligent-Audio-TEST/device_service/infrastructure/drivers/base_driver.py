...
class BaseDeviceDriver:
    ...
    def get_final_results(self, device_sn, task_id=None, test_case_id=None, **kwargs) -> list:
        """所有轮次完成后获取最终聚合结果

        多轮测试场景下，当配置了多轮评估维度时，所有单轮结果采集完毕后
        调用此方法获取跨轮次聚合的最终结果（如合并所有轮次 ASR 日志后
        重新生成的修复结果）。
        ...
        Args:
            ...
            task_id: 任务ID
            test_case_id: 测试用例ID
            **kwargs: 其他参数（如 all_round_results、round_count 等）

        Returns:
            list: 最终结果列表，每项为 dict；默认返回空列表
        """
        return []
