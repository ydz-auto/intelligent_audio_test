...
class XiaoyilivechatV2(HarmonyDriver):
    ...
    def _list_device_mp4_set(self, device_sn):
        """列出设备上所有 mp4 文件路径（用于录屏前后的差异对比）"""
        r = self._hdc_shell(device_sn, 'mediatool', 'query', '.mp4', '-u')
        if r.returncode != 0:
            return set()
        paths = set()
    ...
    def _get_device_file_size(self, device_sn, path):
        """获取设备上指定文件大小"""
        r = self._hdc_shell(device_sn, 'stat', '-c', '%s', path)
        if r.returncode != 0:
            return -1
        try:
    ...
    def pre_process(self, device_sn, task_id=None, test_case_id=None, **kwargs) -> bool:
        ...
        if not self._wait_for_condition(
            ...
            timeout=self.WAIT_LISTENING_TIMEOUT, interval=NORMAL_WAIT,
    ...
    def post_process(self, device_sn, task_id=None, test_case_id=None, **kwargs) -> bool:
        ...
        replied = self._wait_for_condition(
            ...
            timeout=self.WAIT_REPLY_TIMEOUT, interval=NORMAL_WAIT,
        ...
        if replied:
            ...
            self._wait_for_condition(
                lambda: driver.find_component(By.text('正在听…')),
                timeout=self.REPLY_DONE_TIMEOUT, interval=NORMAL_WAIT,
