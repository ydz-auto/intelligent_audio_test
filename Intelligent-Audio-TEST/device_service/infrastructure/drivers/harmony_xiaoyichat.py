...
class Xiaoyilivechat(HarmonyDriver):
    ...
    V10 全量对齐版，在旧版基础上补齐：
    - PCM 抓取：小艺走 DSP 层 audio_hook（替换 fwk 层），豆包/ChatGPT 走 fwk 层
    - RMS 双阶段回复检测（不依赖控件文本）+ barge-in 打断门
    - record_mode=case/round 双模式（case 中间轮不停录屏/不挂断）
    - 停华为音乐/清闹钟/清 pcm 残留、ai PCM 首帧检测
    - 采集文件统一上传 OSS（本地临时目录即传即清）
    """
    ...
    # 小艺聊天页右上角设置入口坐标（SymbolGlyph 选择器在新版本上失效,改用坐标）
    SETTINGS_ENTRY_TOUCH = (1154, 234)
    ...
    def _start_recorder(self, device_sn, file_name=None):
        ...
        if result.returncode != 0:
            ...
            self._record_device_path = None
            return False

        # 轮询：发现新文件 + 等待首帧写入（size 从 0 变非0）
        first_frame_ms = None
        # _start_recorder 发现的真实录屏文件设备路径(VID_*.mp4),供 get_results/_pull_record_file 直接 recv
        self._record_device_path = None
        recorder_timeout_ms = int(config_manager.get_value('device_timing', 'recorder_start_timeout', 30)) * 1000
        deadline = int(time.time() * 1000) + recorder_timeout_ms
        while int(time.time() * 1000) < deadline:
            ...
            if new_paths:
                ...
                if size > 0:
                    ...
                    # 记下真实路径,绕开 mediatool 按 CustomizedFileName 查询
                    self._record_device_path = new_path
                    break
    ...
    def _detect_ai_pcm_first_frame(self, device_sn, app='xiaoyi',
                                   ...
                                   timeout_ms=AI_PCM_FIRST_FRAME_TIMEOUT_MS):
    ...
    def _read_tail_rms(self, device_sn, remote, tail_seconds=1.0,
        ...
        try:
            r = subprocess.run(
                ...
                capture_output=True, timeout=RMS_READ_TIMEOUT,
            )
    ...
    def _scan_remote_for_speech(self, device_sn, remote, seconds=None,
        ...
        """
        if seconds is None:
            seconds = RMS_SCAN_SECONDS
        if energy_thr is None:
            energy_thr = RMS_THRESHOLD
        fmt = self._parse_pcm_fmt(remote)
        if not fmt:
            return False
        sr, ch, sw = fmt
    ...
    def _wait_ai_reply_start_via_pcm(self, device_sn, task_id=None, test_case_id=None,
                                     start_timeout=None, energy_thr=None, interval=NORMAL_WAIT):
        ...
        if start_timeout is None:
            start_timeout = RMS_START_TIMEOUT
        if energy_thr is None:
            energy_thr = RMS_THRESHOLD
        remote = None
        ...
        if remote and self._scan_remote_for_speech(device_sn, remote, seconds=RMS_SCAN_SECONDS,
    ...
    def _wait_ai_reply_end_via_pcm(self, device_sn, task_id=None, test_case_id=None,
        ...
        remote = None
        # 阶段A: 等 AI 开始说话
        deadline_start = time.time() + RMS_START_TIMEOUT
        saw_speech = False
        while time.time() < deadline_start:
            if self._check_stop("post_process_等AI回复开始"):
                return False
        ...
        if not saw_speech:
            ...
            if remote and self._scan_remote_for_speech(device_sn, remote,
                ...
                return True
            self._log(level='INFO',
                      content=f"小艺未回复({RMS_START_TIMEOUT}s 内 AI PCM 尾部无语音能量)",
                      task_id=task_id, test_case_id=test_case_id)
            return False

        # 阶段B: 等 AI 说完(连续 silence_seconds 秒 RMS<阈值)
        silence_since = None
        deadline_end = time.time() + RMS_END_TIMEOUT
        while time.time() < deadline_end:
            ...
            if rms >= RMS_THRESHOLD:
                ...
                if silence_since is None:
                    silence_since = now
                elif (now - silence_since) >= RMS_SILENCE_SECONDS:
                    self._log(level='INFO',
                              content=(f"AI回复结束(尾部静默 {RMS_SILENCE_SECONDS}s, "
        ...
        self._log(level='WARNING',
                  content=(f"等待AI回复结束超时 {RMS_END_TIMEOUT}s"
                           f"(已说过但未静默,视为已回复可能截断)"),
                  task_id=task_id, test_case_id=test_case_id)
        return True
    ...
    def _pull_pcm(self, device_sn, app='xiaoyi', task_id=None, test_case_id=None,
                  round_number=None, local_dir=None):
        """拉取对话 pcm（用户输入/AI 回复）到本地临时目录。

        按 app 参数选择对应 app 的缓存目录与文件后缀:
        - 小艺(xiaoyi):   aibase/cache            用户 cap_client_out.pcm  / AI cap_client_ec_out.pcm
        - 豆包(doubao):   .../com.larus.nova.hm/cache      用户 cap_client_out.pcm / AI client_in..pcm
        - chatgpt:        /data/local/tmp                  用户 cap_client_out.pcm / AI client_in..pcm

        local_dir: 本地临时目录（调用方创建，负责上传/清理）。
        round_number: 本地文件名加轮次前缀 r{round}_ 便于多轮区分, _pcm_to_wav 解析时剥离。
        返回 {'user','ai','user_remote','ai_remote'} 本地路径或 None。
        """
        ...
        ai_suffix = cfg['ai_suffix']
        if not local_dir:
            local_dir = tempfile.mkdtemp(prefix=f'case_pcm_{app}_')
    ...
    # ===================== OSS 本地即传即清 =====================
    def _upload_file_to_oss(self, local_path, oss_key_prefix,
                            task_id=None, test_case_id=None, category='case_result'):
        """上传本地文件到 OSS 并返回访问路径；本地路径无效或上传失败返回 None。"""
        if not local_path or not os.path.exists(local_path):
            return None
        try:
            key = f'{oss_key_prefix}/{os.path.basename(local_path)}'
            storage.save_file(local_path, category, key)
            url = storage.build_path(category, key)
            self._log(level='DEBUG', content=f"OSS上传成功: {url}",
                      task_id=task_id, test_case_id=test_case_id)
            return url
        except Exception as e:
            self._log(level='WARNING', content=f"OSS上传失败: {local_path}: {e}",
                      task_id=task_id, test_case_id=test_case_id)
            return None

    def _pull_pcm_wav_oss(self, device_sn, app='xiaoyi', task_id=None, test_case_id=None,
                          round_number=None):
        """拉取对话 pcm→wav 并上传 OSS（V31 无本地静态目录,采集完即传即清）。

        返回 (user_wav_oss_or_None, ai_wav_oss_or_None)。
        """
        ...
        local_dir = tempfile.mkdtemp(prefix=f'case_pcm_{task_id_path}_{test_case_id_path}_')
        try:
            user_wav, ai_wav = self._pull_pcm_wav(
                device_sn, app=app, task_id=task_id, test_case_id=test_case_id,
                round_number=round_number, local_dir=local_dir)
            user_oss = self._upload_file_to_oss(user_wav, oss_key_prefix,
                                                task_id=task_id, test_case_id=test_case_id)
            ai_oss = self._upload_file_to_oss(ai_wav, oss_key_prefix,
                                              task_id=task_id, test_case_id=test_case_id)
        finally:
            shutil.rmtree(local_dir, ignore_errors=True)
        return user_oss, ai_oss
    ...
    def _pull_dsp_pcm_wav(self, device_sn, task_id=None, test_case_id=None,
                          round_number=None, local_dir=None):
        ...
        self._log(level='DEBUG',
                  ...
                  task_id=task_id, test_case_id=test_case_id)
        if not local_dir:
            local_dir = tempfile.mkdtemp(prefix='case_dsp_')
    ...
    def initialize(self, device_sn, task_id=None, test_case_id=None, **kwargs) -> bool:
        ...
        if user_center:
            ...
            self._log(level='WARNING', content="initialize: 未找到'小艺'入口,聊天窗口可能未打开(后续 pre_process 点通话按钮可能失败)",
                      task_id=task_id, test_case_id=test_case_id)
        # 清除上下文: 进入设置界面→点"清除上下文";再进设置→删除对话记录
        driver.touch(self.SETTINGS_ENTRY_TOUCH)
    ...
    def post_process(self, device_sn, task_id=None, test_case_id=None, **kwargs) -> bool:
        ...
        # case 模式打断轮非末轮：延迟 CASE_BARGE_IN_DELAY_SECONDS 再进下一轮播放（可被停止/暂停打断）
        if (kwargs.get('is_interruption') in (True, 'true', '1', 1)
                and record_mode == 'case' and not is_last):
            self._log(level='INFO',
                      content=f"打断轮结束,等待{CASE_BARGE_IN_DELAY_SECONDS}s后进入下一轮播放: r{round_number}/{total_rounds}",
                      task_id=task_id, test_case_id=test_case_id)
            poll = CHECK_STOP_POLL_INTERVAL
            for _ in range(max(1, int(round(CASE_BARGE_IN_DELAY_SECONDS / poll)))):
                if self._check_stop('case轮间延迟'):
                    return True
                time.sleep(poll)
    ...
    def _pull_record_file(self, device_sn, task_id=None, test_case_id=None):
        """从设备拉取录屏文件(转 wav)并上传 OSS，返回 mp4 的 OSS URL（失败返回 None）。teardown 兜底用。

        本地临时目录即传即清: mp4 + wav 都上传 OSS,评估端回看用 mp4、分析用 wav。
        """
        ...
        test_case_id_path = str(test_case_id) if test_case_id else 'default_id'
        oss_key_prefix = f'{task_id_path}/{test_case_id_path}/{device_sn}'
        local_dir = tempfile.mkdtemp(prefix=f'case_record_{task_id_path}_{test_case_id_path}_')
        ...
        if not device_path:
            try:
                ...
                if 'uri' in query.stdout and len(lines) > 2:
                    # 设备端把媒体库文件拷到 shell 可读目录,再从该路径 file recv 回本地
                    self._hdc_shell(device_sn, 'mediatool', 'recv',
                                    lines[2].strip(), DEVICE_TMP_DIR)
        ...
        try:
            ...
            recv_result = subprocess.run(['hdc', '-t', device_sn, 'file', 'recv', device_path, local_path],
                                         ...
                                         timeout=EXTRA_LONG_HDC_TIMEOUT)
            if not os.path.exists(local_path):
                self._log(level='WARNING',
                          content=f"teardown兜底拉取录屏失败: device_path={device_path!r} err={recv_result.stderr}",
                          task_id=task_id, test_case_id=test_case_id)
                return None
            wav_path = self._mp4_to_wav(local_path, task_id=task_id, test_case_id=test_case_id)
            # mp4 + wav 都上传 OSS,返回 mp4 的 OSS URL(评估主视频)
            oss_url = self._upload_file_to_oss(local_path, oss_key_prefix,
                                               task_id=task_id, test_case_id=test_case_id)
            if wav_path:
                self._upload_file_to_oss(wav_path, oss_key_prefix,
                                         task_id=task_id, test_case_id=test_case_id)
            return oss_url
        except Exception as e:
            self._log(level='WARNING', content=f"teardown兜底拉取录屏异常: {e}",
                      task_id=task_id, test_case_id=test_case_id)
            return None
        finally:
            shutil.rmtree(local_dir, ignore_errors=True)
    ...
    def get_results(self, device_sn, task_id=None, test_case_id=None, **kwargs) -> list:
        ...
        if not getattr(self, '_record_enabled', True):
            user_wav, ai_wav = self._pull_pcm_wav_oss(
                device_sn, app=getattr(self, '_pcm_app', 'xiaoyi'),
                task_id=task_id, test_case_id=test_case_id,
        ...
        # 拉取对话 pcm 并转 wav 上传 OSS（用户输入 + AI 回复），抓取目标 app 由 _pcm_app 指定
        pcm_app = getattr(self, '_pcm_app', 'xiaoyi')
        user_wav, ai_wav = self._pull_pcm_wav_oss(device_sn, app=pcm_app,
                                                  task_id=task_id, test_case_id=test_case_id,
                                                  round_number=getattr(self, '_round_number', None))
        # 录屏 mp4: 本地临时目录即传即清(拉取→转 wav→mp4+wav 上传 OSS)
        local_dir = tempfile.mkdtemp(prefix=f'case_record_{task_id_path}_{test_case_id_path}_')
        try:
            ...
            # mp4 无损转 wav
            wav_path = self._mp4_to_wav(local_path, task_id=task_id, test_case_id=test_case_id)
            # mp4(回看原视频) + wav(评估分析) 都上传 OSS
            record_path = self._upload_file_to_oss(local_path, oss_key_prefix,
                                                   task_id=task_id, test_case_id=test_case_id)
            wav_oss = self._upload_file_to_oss(wav_path, oss_key_prefix,
                                               task_id=task_id, test_case_id=test_case_id) if wav_path else None
            if not record_path:
                self._log(level='ERROR', content=f"录屏文件 OSS 上传失败: {local_path}",
                          task_id=task_id, test_case_id=test_case_id)
            self._record_pulled = True
            self._log(level='INFO',
                      content=f"录屏处理完成: record_path={record_path} wav_path={wav_oss}",
                      task_id=task_id, test_case_id=test_case_id)
            return [{
                'success': True,
                'message': 'Success',
    ...
    def teardown(self, device_sn, task_id=None, test_case_id=None, **kwargs) -> bool:
        ...
        2. 确保通话已挂断（兜底）
        3. 退出小艺聊天界面，回桌面
        4. 清掉华为音乐 5. 清空闹钟 6. 停止小艺 APP 7. 清理 pcm 缓存
        """
        ...
        if getattr(self, '_record_mode', 'round') == 'case' and not getattr(self, '_record_pulled', False):
            ...
            if pulled:
                self._log(level='INFO', content=f"teardown: 兜底拉取录屏成功(OSS): {pulled}",
                          task_id=task_id, test_case_id=test_case_id)
                self._record_pulled = True
