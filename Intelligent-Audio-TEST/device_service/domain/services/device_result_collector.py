...
from device_service.domain.services.rttm_stm_utils import RttmStmUtils


class DeviceResultCollector:
    """设备结果采集器基类"""

    def __init__(self, field_query_service=None):
        """初始化

        Args:
            field_query_service: 字段查询服务（ACL repository），
                提供 get_mapped_device_output_fields / convert_device_output /
                get_mapped_device_output_field_keys / get_algorithm_extra_config。
                None 时 convert_results / build_case_result_log 退化为空映射。
        """
        self._field_query = field_query_service
        self._aligner = TimestampAligner(field_query_service=field_query_service)
        self._rttm_utils = RttmStmUtils(field_query_service=field_query_service)
    ...
    def convert_results(self, all_results, algorithm_type):
        ...
        for res in all_results:
            ...
            if self._field_query:
                mapped_fields = self._field_query.get_mapped_device_output_fields(algorithm_type)
                log_not_emit('DEBUG', 'device_collector', f'mapped_fields: {[f.code for f in mapped_fields]}',
                             category='engine')

                mapped_results = self._field_query.convert_device_output(algorithm_type, raw_results)
            else:
                mapped_results = {}
    ...
    def build_case_result_log(self, algorithm_type, res, ref_fields=None, **kwargs):
        ...
        # 获取算法映射后的设备输出字段键列表
        if self._field_query:
            mapped_output_keys = self._field_query.get_mapped_device_output_field_keys(algorithm_type)
        else:
            mapped_output_keys = []
        ...
        # 处理额外配置的查询字段
        if self._field_query:
            extra_fields = self._field_query.get_algorithm_extra_config(algorithm_type).get('query_fields', {}).keys()
        else:
            extra_fields = []
...
def get_device_result_collector(field_query_service=None):
    """获取设备结果采集器实例

    Args:
        field_query_service: 字段查询服务（ACL repository），可选
    """
    return DeviceResultCollector(field_query_service=field_query_service)
