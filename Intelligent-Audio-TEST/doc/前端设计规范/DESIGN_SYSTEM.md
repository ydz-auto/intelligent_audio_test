...
- 设计时充分考虑HTML可视化能力、Tailwind CSS响应式模式，以及前端View层、前端Controller逻辑和Flask+SQLite后端服务之间的数据流
...
### 特殊处理
- 关键模块（SQLite延迟指示器、Flask接口状态）接收微妙渐变（从 #FF6A00/5 到 #FF6A00/10 或从 #1677FF/5 到 #1677FF/10），增加深度而不会使简洁美学过于复杂
...
### 进程分离
- **主进程 (Main Process)**：负责后端逻辑、数据库操作(SQLite)、原生API调用
...
5. **后端处理**：主进程调用 Flask API 或直接操作 SQLite 数据库
