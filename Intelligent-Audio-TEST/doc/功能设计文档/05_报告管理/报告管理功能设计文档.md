...
| 报告对比 | 比较多个测试报告结果的过程 |
| 对比报告 | 基于多个任务（taskIds）生成的横向对比报告 |
| 二次对比报告 | 基于已有报告（reportIds）再次生成的对比报告 |
...
报告管理功能采用前后端分离架构，整体遵循 DDD + CQRS 思想：写侧负责报告生成与落库（命令），读侧负责列表/详情/用例检索等查询（查询），读写模型分离。

**前端展示层**：基于 Vue 3 + Composition API + Pinia 构建，遵循 DDD 分层——Presentation（views/components）只见 camelCase Domain 与 composables（Ports）；Application（composables/store）编排用例；Infrastructure（api/adapters/dto）唯一感知 snake_case 并完成 DTO→Domain 映射。报告生成进度通过进度查询接口轮询获取。

**后端服务层**：基于 Flask + SQLAlchemy 实现（蓝图统一注册于 `/api/v1/reports` 前缀），负责报告生成、存储管理、对比分析和统计计算。报告生成采用异步任务 + 进度查询接口；指标计算统一在后端完成（聚合策略配置化）。

**报告存储层**：PostgreSQL 报告 7 表（主表 + 按需加载扩展表，见第 3.1 节），摘要小字段与明细大字段分层存储，列表查询仅读摘要表。

### 2.2 核心组件

| 组件 | 职责 | 技术实现 | 通信方式 |
|------|------|----------|----------|
| 前端报告管理页面 | 报告列表、详情查看、对比与导出入口 | Vue 3 + Pinia + Element Plus + ECharts | HTTP REST |
| 报告 API 蓝图 | 路由注册与请求校验 | Flask Blueprint（`backend/blueprints/report_bp.py`，前缀 `/api/v1/reports`）+ Pydantic Schema | HTTP REST |
| 报告生成器 | 任务报告异步生成与落库 | `report_controller_task.py` + `report_utils.py`（聚合策略配置化） | 异步任务 + 进度查询 |
| 报告对比 | 任务对比 / 二次对比报告生成 | `report_controller_compare.py` / `report_controller_secondary.py` | HTTP REST |
| 报告存储 | 报告主表 + 摘要/元数据/原始数据/用例/统计/矩阵 7 表 | SQLAlchemy + PostgreSQL | 数据库 |
| 报告导出 | 报告导出为文件并下载 | Flask `send_file`（HTML 导出） | HTTP REST |
...
**导出格式**（`POST /api/v1/reports/export`，参数 ids + format）
- CSV格式：批量导出报告摘要（报告ID、名称、类型、生成时间、总用例数、成功率、分析结论），支持多报告
- HTML格式：单个报告完整内容的所见即所得导出（后端 `HtmlReportRenderer` 服务端渲染）

**导出选项**
- CSV 支持批量选择多个报告；HTML 仅支持单个报告
- 导出文件按报告 ID 与名称自动命名
- 导出内容为报告落库数据（报告 7 表）的快照，由后端统一生成，前端不做二次计算
...
> 注：报告模板管理为规划能力，当前版本未实现（系统中不存在 ReportTemplate 模型与模板管理接口）；报告结构由后端报告生成器与 `HtmlReportRenderer` 固定生成。
...
export const useReportComparison = () => {
  ...
  const executeComparison = async () => {
    ...
    try {
      // 对比报告基于任务列表生成（实际载荷 { taskIds, name }）；
      // 二次对比基于已有报告：POST /api/v1/reports/secondary-compare，载荷 { reportIds }
      const response = await axios.post('/api/v1/reports/compare', {
        taskIds: selectedReports.value,
        name: '对比报告'
      });
...
- **框架**：Flask（Flask-SocketIO 仅用于日志推送，不用于报告）
- **数据库**：PostgreSQL + SQLAlchemy
- **网络通信**：RESTful API（报告进度轮询获取）
- **数据校验**：Pydantic Schema（请求参数模型化校验，拒绝魔法字符串）
- **报告生成**：`HtmlReportRenderer`（HTML 所见即所得渲染） + 聚合策略配置化（`aggregation_strategies.py`）
- **数据处理**：Pandas（CSV 批量导出）
- **图表生成**：ECharts（前端渲染）
