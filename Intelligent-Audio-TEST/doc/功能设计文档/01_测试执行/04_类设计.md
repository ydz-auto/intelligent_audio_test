...
### 5.2 HttpAPIAdapter — HTTP 适配器

> 源码：`backend/utils/clients/http_api_adapter.py`

**职责**：HTTP 请求-响应模型适配器。`send()` 返回结构与 `APIDriver.execute()` 完全一致，消费方无缝迁移。
...
| (protocol, vendor) | 适配器 |
|--------------------|--------|
| `('http', 'volc_ast')` | `VolcASTAdapter` |
| `('http', 'qwen_llm')` | `QwenLLMAdapter` |
| `('http', 'default')` | `HttpAPIAdapter` |
| `('websocket', 'openai')` / `('websocket', 'openai_realtime')` | `OpenAIRealtimeAdapter` |
...
class APIAdapterFactory:
    ...
    def get_adapter(self, api_config, case_config=None,
        ...
        # 2. protocol：case_config.protocol → endpoint 前缀 (ws://wss://) → 默认 http
        # 3. 查注册表；websocket 未注册抛 ValueError，http 未注册回退 default
        ...

api_adapter_factory = APIAdapterFactory()   # 单例
```

### 5.6 AudioStreamOrchestrator — 多源混音切片

> 源码：`backend/services/audio/audio_stream_orchestrator.py`

**职责**：多源音频流编排：主讲人 + 干扰人 + 噪声 → 混音 → PCM chunk。
复用 `audio_timeline.calculate_speaker_aware_audio_delays()` 的时间轴算法，
输出 base64 PCM chunk 列表供 Realtime adapter 流式推送。

**与 E2E 的区别**
- E2E：多路音频 → PyAudioDriver callback 实时混音 → 物理设备播放；
- API 类（非实时/流式/Realtime）：多路音频 → 离线混音 → 切片 → 整文件 POST / SSE chunk / WS chunk。

**噪声合并优先级**（与 E2E playback_orchestrator 一致）：轮次级 `background_noise` > case 级 `background_noise`；有轮次噪声 → 混轮次噪声；有全局噪声 → 混全局噪声；俩都有 → 混轮次噪声。

**混音步骤（6 步）**
1. 加载各源 PCM（主讲人/干扰人/噪声，源格式各异）；
2. 格式适配：`AudioFormatAdapter` 统一到 `api.audio_config` 目标格式（位深→重采样→通道，不同采样率样本无法直接叠加）；
3. RMS 补偿 + SPL 增益：所有源归一化后按 SPL 映射调整；
4. 时间轴编排：`calculate_speaker_aware_audio_delays` 计算交叠；
5. 混音叠加：主讲人 + 干扰人 + 噪声按 start_time 叠加到混音缓冲（噪声 loop=True 铺满缓冲）；
6. 切片输出：按目标位深量化 → 按 `chunk_duration_ms` 切分为 base64 PCM chunk（不足补零）。

**常量**（配置化，拒绝魔法数字）

| 常量 | 值 | 说明 |
|------|-----|------|
| `DEFAULT_SAMPLE_RATE` | 24000 | `api.audio_config` 未配置时回退（OpenAI 兼容） |
...
class AudioStreamOrchestrator:
    @staticmethod
    def chunk_audio(audios_config, audio_loader, chunk_duration_ms=100,
                    api_id=None, rms_spl_mapping=None,
                    background_noise=None, target_format=None) -> list[str]:
        # audios_config: [{audio_id, type, delay, play_order, spl, ...}]（主讲人+干扰人）
        # audio_loader: 加载 PCM 的回调 (audio_id) -> bytes
        # chunk_duration_ms: 切片时长（ms），默认 100
        # api_id: 被测 API ID（查 RMS→SPL 映射）
        # rms_spl_mapping: 预加载的 ApiRmsSplMapping（可选，避免重复查 DB）
        # background_noise: 噪声配置（已按优先级合并）或 None
        # target_format: {'sample_rate', 'bit_depth', 'channels'}（None → 默认 24k/s16/mono）
        # 返回: base64 编码的 PCM chunk 列表（消费方式由 executor 决定）
        ...

    @staticmethod
    def _rms_compensate(pcm_bytes, fmt) -> bytes:
        # RMS 补偿：gain_db = TARGET_RMS_DBFS - current_rms_db，归一化到目标
        ...
```

### 5.6a AudioFormatAdapter — 音频格式适配器
...
> **与 orchestrator 的协作**：`AudioStreamOrchestrator.chunk_audio(target_format=api.audio_config)`
> Step 2 对每个源调用 `detect_source_format` + `adapt`，混音在目标格式下进行，
...
### 5.7b 混音步骤流程图（含格式适配，三种 API 模式通用）
...
│                          │                                     │
│                          ▼                                     │
│  ┌─ Step 6: 按目标位深切片 — base64 chunk ───────────────┐     │
│  │  6a. 量化到目标位深: quantize(mix_buffer, s16)          │     │
│  │      → np.clip(-1, 1) × 32767                          │     │
│  │  6b. 切片:                                              │     │
│  │      chunk_samples = 24kHz × 100ms = 2400 samples      │     │
│  │      chunk_bytes = 2400 × bytes_per_sample(s16) = 4800 │     │
│  │      mix_buffer[0:2400]     → base64 → chunk_0         │     │
│  │      mix_buffer[2400:4800]  → base64 → chunk_1         │     │
│  │      ...                                                │     │
...
### 5.7c E2E vs API 类 混音/SPL 对照（非实时/流式/Realtime 通用）

| 维度 | E2E (audio_driver + spl_service) | API 类 (AudioStreamOrchestrator + ApiRmsSplService) |
|------|----------------------------------|-----------------------------------------------------|
| 适用模式 | 物理设备播放（speaker） | **HTTP 非实时 + HTTP 流式（SSE）+ Realtime（WS）三种通用** |
| 混音时机 | 实时（PyAudio callback 内逐帧混音） | 离线整段混音（执行中一次性混好整段） |
| 混音实现 | `_create_multi_callback` 内 `+=` 叠加 | `np.zeros` + `+=` 叠加 |
| 格式适配 | 不需要（声卡/驱动负责格式转换） | **`AudioFormatAdapter.adapt()` — 统一到 `api.audio_config`** |
| RMS 补偿 | `calculate_gain_compensation` (file→dBFS→gain) | `_rms_compensate` (bytes→float32→dBFS→gain→quantize) |
...
| 输出 | int16 → PyAudio stream | 按目标位深 quantize → base64 → 消费方式分叉 |
| 消费方式 | — | 整文件 POST / SSE chunk / WS chunk（仅此不同） |
| 防削波 | `np.clip(-32768, 32767)` | `np.clip(-1, 1)` 归一化域，量化时裁剪 |
...
class RealtimeSessionExecutor:
    ...
    def _execute_normal_round(self, adapter, round_config, round_idx,
        ...
        # ① chunk_audio 混音 + 逐 chunk 推送（间隔 adapter._chunk_duration_ms）
...
classDiagram
    ...
    RealtimeSessionExecutor ..> AudioStreamOrchestrator : chunk_audio
    AudioStreamOrchestrator ..> AudioFormatAdapter : adapt
    AudioStreamOrchestrator ..> ApiRmsSplService : spl_to_gain
```