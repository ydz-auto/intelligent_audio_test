...
评估服务在构建 payload 时，需要将设备参数、API 参数、参考参数中的音频文件内容传递给评估服务器。传统做法是将文件读取后编码为 base64 data URI 内联到 JSON payload 中，存在以下风险：

- **内存爆炸**：`f.read()` 全量读取文件到内存，base64 编码后体积膨胀约 33%，大文件（如 100MB 音频）会导致内存占用超过 130MB
- **日志爆炸**：payload 直接写入日志时，base64 字符串会导致单条日志体积巨大，影响日志系统性能和磁盘空间

#### 6.3.2 大文件流式上传方案

评估 API 客户端（`evaluation_api_client.py`）实现了自动切换 JSON/multipart 的上传机制：

**大小限制**：设置 `MAX_INLINE_FILE_SIZE = 20MB` 常量，超过此大小的文件不内联到 JSON 中。

**文件读取（`_read_file_as_data_uri`）**：
- 小文件（≤20MB）：读取内容并编码为 `data:audio/wav;base64,...` 格式，内联到 JSON payload
- 大文件（>20MB）：返回 `__file_ref__:<文件路径>` 标记，不读取文件内容到内存

**自动切换（`make_api_request`）**：
- 检测 payload 中是否包含 `__file_ref__:` 标记（`_has_file_refs` 递归检测）
- 包含标记时，调用 `_split_payload_for_multipart` 将 payload 拆分为 `data`（普通字段）和 `files`（文件流句柄），使用 `requests.post(url, data=data, files=files)` 流式上传
- 不包含标记时，保持原有 JSON 方式 `requests.post(url, json=payload)`
- `finally` 块中关闭所有文件句柄，防止资源泄漏

**优势**：
- 流式上传从磁盘逐块读取，内存占用恒定，不受文件大小影响
- 小文件保持 JSON 内联，兼容性好，无需服务端额外处理
- 自动切换对调用方透明，无需修改上层逻辑

#### 6.3.3 日志安全防护

**问题**：payload 中可能包含 base64 编码的音频数据，直接写入日志会导致日志爆炸。

**解决方案**：新增 `_truncate_payload_for_log` 静态方法，在写入日志前截断超长值：

- 字符串值超过 200 字符时，截取前 80 字符并附加 `...(N chars)` 标注原始长度
- 递归处理嵌套 dict 结构
- 所有日志输出 payload 的位置均使用截断后的副本
...
```mermaid
flowchart TD
    A[构建评估 payload] --> B[读取文件参数]
    B --> C{文件大小 > 20MB?}
    C -->|是| D[返回 __file_ref__ 标记]
    C -->|否| E[编码为 base64 data URI]
    D --> F{payload 含 __file_ref__?}
    E --> F
    F -->|是| G[拆分为 data + files]
    G --> H[multipart/form-data 流式上传]
    F -->|否| I[JSON 内联上传]
    H --> J[finally 关闭文件句柄]
    I --> J
```
