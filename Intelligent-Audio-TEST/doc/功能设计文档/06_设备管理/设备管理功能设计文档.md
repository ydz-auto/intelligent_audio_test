...
> **范围界定（2026-09 对齐《01_测试执行》最新设计）**
>
> 本文档聚焦**物理设备**（Android/鸿蒙真机、播放设备）的管理。最新设计中被测设备按 `device_type`
> 分为三类：`physical` / `http_api` / `websocket_api`（枚举化，见《01_测试执行/05_路由与废弃.md》），同一任务内不同用例可混跑：
>
> - `physical`：使用本文档管理的真机设备，由 `E2EExecutor` 执行端到端测试；
> - `http_api` / `websocket_api`：API 类被测设备**不经过物理设备管理**，由 `ExecutionEngine` 路由到
>   `APISessionExecutor` / `RealtimeSessionExecutor`，经 API Adapter 体系（`BaseAPIAdapter` →
>   `HttpAPIAdapter` / `HttpStreamAdapter` / `RealtimeAPIAdapter`）调用，详见《多厂商API适配完整方案.md》顶部状态更新；
> - App 侧驱动（被测 App 的自动化驱动）由 `backend/utils/device_driver/` 的注册表机制统一管理，见本文 3.8 节。
...
| 设备驱动（AppDriver） | 被测 App 的自动化驱动实现，声明 `app_type` / `version` / `platform` / `display_name`，见 `backend/utils/device_driver/contracts/app_driver.py` |
| 驱动注册表（DriverRegistry） | 按 `(AppType, AppVersion, DevicePlatform)` 三元 key 管理设备驱动的注册、依赖检测、版本降级 resolve 与热更新 |
| 驱动状态（DriverStatus） | 驱动可用性枚举：`AVAILABLE` / `MISSING_DEPENDENCY` / `DISABLED`（`backend/utils/device_driver/driver_types.py`） |
| 被测设备类型（device_type） | 被测设备类型枚举：`physical` / `http_api` / `websocket_api`，`ExecutionEngine` 据此路由执行器 |
...
| 设备驱动注册表（DriverRegistry） | 管理设备驱动（AppDriver）的注册、依赖检测、版本降级 resolve 与热更新，为 `E2EExecutor` 提供驱动实例 | Python + `backend/utils/device_driver/registry.py` | 进程内单例调用 |
...
> **枚举化约定（对齐 DDD 分层）**：上述连接状态为 **DeviceStatus 枚举**（连接态），与 `backend/utils/device_driver/driver_types.py`
> 中的 **DriverStatus 枚举**（`AVAILABLE` / `MISSING_DEPENDENCY` / `DISABLED`，描述驱动可用性）是两个不同维度的状态机，不得混用。
> 状态值一律以枚举承载、禁止魔法字符串：前端 Domain 层 `enums.ts` 使用 camelCase，snake_case 仅存在于
> Infrastructure 层（api/ + adapters/ + dto/ + http/），由 DTO 转换负责映射。
...
### 3.8 设备驱动注册与解析（backend/utils/device_driver）

本节增补自当前后端实际实现（2026-09 核实），描述 App 侧设备驱动的注册表机制。它与本文前述的
设备连接/分组/播放管理互补：前者管理"驱动"（如何自动化被测 App），后者管理"设备"（真机与播放设备资源）。

#### 3.8.1 驱动注册表（DriverRegistry）

- **注册方式**：驱动类通过 `@register_driver` 装饰器在模块导入时自动注册进 `DriverRegistry`（`registry.py`），
  注册表 key 为三元组 `(AppType, AppVersion, DevicePlatform)`，全程枚举、无魔法字符串；注册表以 `RLock` 保证线程安全，
  模块级单例 `driver_registry` 供全局访问。
- **能力/依赖声明**：驱动按 `AppDriver` 契约声明 `app_type` / `version` / `platform` / `display_name`（必填）与
  `dependencies` / `fallback_version`（可选）。注册表用 `importlib.util.find_spec` 做依赖检测，依赖缺失时驱动状态置为
  `MISSING_DEPENDENCY`（`DriverStatus` 枚举：`AVAILABLE` / `MISSING_DEPENDENCY` / `DISABLED`）。
- **版本降级 resolve**：`resolve()` 按四步查找：① 三元 key 精确匹配 → ② 类声明的 `fallback_version` →
  ③ 自动降级到 `AppVersion.V1` → ④ 任意可用版本；全部失败时抛出明确异常。
- **热更新**：`reload_module` 支持不重启服务更新驱动（卸载模块 → 重新导入 → 刷新注册表）。
- **当前已注册驱动**：Android/鸿蒙通用驱动、Plaud、豆包 ASR / 豆包 Chat、小艺系列（Face2Face、同声传译、会记、
  Live Chat、输入法）、ChatGPT Voice Chat 等 10 余个（见各驱动模块的 `@register_driver`）。

#### 3.8.2 驱动契约与工厂

- **AppDriver 契约**（`contracts/app_driver.py`）：抽象方法 `initialize` / `get_results`；默认实现
  `pre_process` / `post_process` / `teardown` / `is_locked` / `set_volume` / `scan` /
  `extract_results_from_archive` / `get_final_results`。`BaseDeviceDriver` 为 legacy 兼容基类，
  提供相同生命周期并附加 `_check_stop` / `_wait_for_condition` / `_extract_playback_timestamps` 等工具方法。
- **DeviceDriverFactory**（`driver_factory.py`）：`get_driver(system, keywords)` 沿 legacy 关键字表路由
  （专用驱动优先，关键字映射集中于 `_LEGACY_KEYWORDS` 常量表，无散落硬编码）；`get_driver_typed(...)`
  走上述新注册表。例如鸿蒙 `hypium` 依赖缺失时，相关鸿蒙驱动不可用（工厂返回空）。
- **与测试执行的关系**：`E2EExecutor` 通过 `device_driver_factory.get_driver` 获取驱动执行用例
  （机制描述见《01_测试执行/任务重新评估与重新提取功能设计文档.md》）。
- **与 API 适配器注册的区分**：`DriverRegistry` 只管设备驱动；API 被测服务的适配器由 `APIAdapterFactory`
  按 `(protocol, vendor)` 或 `adapter_class` 创建（设计见《01_测试执行/04_类设计.md》），两套注册表互不混用。
- **扩展原则（配置化/开放-封闭）**：新增被测 App 只需新增一个 `@register_driver` 驱动类并声明三元 key，
  不修改执行器与工厂代码；`AppType` / `AppVersion` / `DevicePlatform` 均以枚举维护，新增取值集中在
  `driver_types.py`。
...
- **插件系统**：支持第三方插件扩展功能
- **设备驱动管理**：已由 `backend/utils/device_driver/` 的 `DriverRegistry` 承接（`@register_driver` 装饰器注册、
  依赖检测、`resolve()` 版本降级、`reload_module` 热更新，见本文 3.8 节）；后续可补充驱动的可视化运维界面
  （注册表状态展示、依赖体检、热更新触发）
