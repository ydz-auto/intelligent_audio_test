# 10 - reference_params_generator 适配（上传生成文件，存路径）

## 涉及文件
- `Intelligent-Audio-TEST/backend/algorithm/reference_params_generator.py`
...
## 改造方案（上传生成文件，config 存路径，用户可编辑）

### 核心变更

```
旧流程：音频标注 → 聚合 → 写入 TestCase.reference_params 列（MB级）
新流程：上传音频 → 按轮从 AudioAnnotation 生成 → 写入文件 → 路径存入 rounds[].referenceParamsPath
```
...
def on_audio_associated(self, test_case_id, round_number, audio_id):
    ...
    # 5. 路径存入 config
    self._update_round_path(test_case_id, round_number, file_path)
```
...
def _write_ref_file(self, test_case_id, round_number, content):
    """写入参考文本文件"""
    file_path = f"/data/ref/{test_case_id}_round_{round_number}.json"
...
```
前端读取 referenceParamsPath → 请求后端读取文件内容 → 展示给用户
...
### 废弃 TestCase.reference_params 列

| 变更 | 说明 |
|------|------|
| `TestCase.reference_params` | **废弃** — 保留列，不再写入 |
| `rounds[].referenceParamsPath` | **新位置** — 文件路径（字符串） |
...
| 场景 | 旧设计 | 新设计 |
|------|--------|--------|
| config 体积 | 无 reference_params | 只有路径字符串 |
| DB 列 | MB 级 reference_params | 废弃 |
| 文件 | 无 | MB 级文件（不影响 config） |
