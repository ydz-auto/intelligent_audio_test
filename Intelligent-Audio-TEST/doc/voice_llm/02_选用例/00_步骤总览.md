...
已采用**双记录架构**：一个逻辑用例拆分为 API 和 E2E 两条独立数据库记录，通过 `related_case_id` 互指。每条记录的 config 是扁平的。
...
| `models.py` | TestCase 模型（含 test_type/related_case_id） | 已完成 |
...
| 改造项 | 状态 |
|--------|------|
| TestCase 模型新增 `test_type`、`related_case_id` 字段 | 已完成 |
| DDL 迁移脚本 (`split_testcase_by_type.py`) | 已完成 |
| testcase_controller 双记录 CRUD（创建/更新/查询/删除/复制） | 已完成 |
| testcase Schema 新增 `related_case_id`、`test_type` 字段 | 已完成 |
...
| ↳ 每轮包含 | 音频配置、噪声、评估、参考字段(路径)、算法参数（含inputText/inputAudio/waitTime/railDistance/volumeLevel/voiceprintEnabled/promptAudioId/interferers/interruptionEnabled等） | — |
...
| 算法参数 | algorithmParams[{field_code,field_value}] | 是 | 是 | 含inputText/inputAudio/waitTime/railDistance/volumeLevel/voiceprintEnabled/promptAudioId/interferers/interruptionEnabled等 |
...
| 2h | **打断检测** | `round.algorithmParams[interruptionEnabled]` | 可选 | enabled 时监听麦克风 |
| 2i | 收集本轮结果 | — | 必须 | 已有 |
| 2j | **单轮评估** | `round.evaluation` + 读取 `referenceParamsPath` | 可选 | enabled 时独立评估 |
| 2k | **恢复音量/导轨** | — | 可选 | 本轮设了才恢复 |
| 3 | 后处理 + 采集结果 | — | 必须 | 已有 |
| 4 | 结果存储 + 整体评估 | — | 必须 | 已有 |
