...
- **全局噪声桥接（未实现）**：V9.7.10 会在 `start/stop_background_noise` 成功后回写 `global_noise_start_ms`/`global_noise_end_ms` 到 `self._playback_timestamps[task_id]`；V9.7.31 无此桥接，详见 §7 差异 1/3。

### 3.5 参数透传（e2e_collector.py + e2e_calculation_service.py）

`collect_results()` 在 `extra_params` 注入（L48-67）：

1. `playback_time_offsets`：`E2ECalculationService.calculate_actual_offset`（domain/services/e2e_calculation_service.py L30-67）计算；**key 仍为 `{audio_id}_{play_order}`**（L60，无 audio_type 前缀）；噪声/干扰人跳过对齐基准（L49），取值端只消费 `values[0]['offset']`，key 格式无功能影响。
2. `playback_start_time_ms`/`playback_end_time_ms`（轮级，L53-55）与 `playback_timestamps_detail`（L59-67）：结构见 §4。
3. **`global_noise_timestamps`（未实现）**：V9.7.10 会在 L67 附近额外注入用例级字段；V9.7.31 无此注入，详见 §7 差异 3。
...
> **注意（V9.7.31 现状）**：注入明细**不含 `audio_type`** 字段（账本 `audio_play_times` 中已收集，注入推导时丢弃）；设备侧目前只能靠 play_order 与条目数量区分段类型，详见 §7 差异 2。

### 4.2 global_noise_timestamps（用例级，跨所有轮次）— 未实现
...
V9.7.31 目标结构如上（对齐 V9.7.10 契约），当前未注入。
...
| 全局噪声播放中逐轮采集 | **V9.7.31 无 `global_noise_timestamps`**，设备驱动暂以轮级 `playback_start_time_ms` 起始的账本推断；补齐打点后应用 `start_ms` + 本轮 `playback_end_time_ms` 界定噪声段 |
| 全局噪声未配置/未启动 | `start_background_noise` 直接返回 True，不影响原链路 |
| `start_background_noise` 重复调用 | 按设备分组重建 player 注册（V9.7.31 无"首次 start_ms 不覆盖"保护，因未打点） |
...
- 每轮 `playback_timestamps_detail`（+ 轮级 `playback_start_time_ms`/`playback_end_time_ms`）→ 定位录音中各段音频窗口；补齐 audio_type 后可按段类型（dry/noise/interferer）精确切分；
- `playback_time_offsets` → 计算实际播放偏移，用于时间对齐（仅 dry 参与基准）；
- 补齐规划中的 `global_noise_timestamps` → 界定跨轮次全局噪声段。

## 7. 与 V9.7.10 的实现差异（待补齐项）

| # | 能力 | V9.7.10 | V9.7.31 现状 | 影响 |
| --- | --- | --- | --- | --- |
| 1 | 全局噪声打点 | `start/stop_background_noise` 打 `start_ms`/`end_ms`，提供 `get_background_noise_timestamps(task_id)` | 均未打点、无查询接口 | 无法界定用例级噪声段 |
| 2 | detail 含 audio_type | `playback_timestamps_detail` 每条含 `audio_type` | 账本已收集但注入明细丢弃 | 设备侧无法按段类型切分 |
| 3 | `global_noise_timestamps` 注入 | `collect_results` 额外注入用例级字段 | 无注入（executor 亦无桥接回写） | 设备侧缺少全局噪声契约字段 |
| 4 | offset key 前缀 | `{audio_type}_{audio_id}_{play_order}` 防键冲突 | 仍 `{audio_id}_{play_order}` | 取值端只消费 `values[0]['offset']`，暂无功能影响；同一音频复用为多类型时存在理论键冲突 |
...
补齐 1-3 的最小改动面：`playback_orchestrator.py` 启停方法打点 + 查询接口；`rounds_loop_mixin.py` 注入时补 `audio_type`、启停后回写桥接；`e2e_collector.py` 注入 `global_noise_timestamps`。
