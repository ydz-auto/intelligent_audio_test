...
- **eval_server**：纯计算服务，接收评估任务（WER/BLEU/LLM Judge/DER），异步返回结果
...
flowchart TD
    ...
    E --> F[for 轮次循环]
    F --> G["构建轮次请求<br/>文本→algorithmParams(inputText)<br/>音频→audios[]"]
    G --> H[发送到 api_adapter_service]
    H --> I[等待响应]
    I --> J[提取轮次结果]
    J --> K[更新上下文]
...
flowchart TD
    ...
    J -->|否| F1[现有单次流程]
    J -->|是| K[for 轮次循环]
    K --> K0["前置准备(从 round.algorithmParams 读取)"]
    K0 --> K1{algorithmParams.railDistance?}
    K1 -->|有| K2[move_rail 到目标距离]
    K1 -->|无| K3
    K2 --> K3{algorithmParams.volumeLevel?}
    K3 -->|有| K4[set_volume 设置设备音量]
    K3 -->|无| K5
    K4 --> K5{algorithmParams.voiceprintEnabled?}
    K5 -->|是| K6[声纹注册]
    K5 -->|否| K7
    K6 --> K7["构建音频列表<br/>干声→round.audios[]<br/>噪声→round.backgroundNoise(loop)<br/>干扰人→algorithmParams.interferers"]
    K7 --> L["play_overlap→play_multi<br/>(按设备分组统一混音)"]
    L --> M[等待设备响应完成]
    M --> N[_collect_round_results]
    N --> O{algorithmParams.interruptionEnabled?}
    O -->|是| P[记录打断事件]
    O -->|否| Q
    P --> Q{evaluationEnabled?}
    Q -->|是| R["入队单轮评估(round_number)"]
    Q -->|否| S
    R --> S[stop_task_audio]
    S --> T{还有下一轮?}
    T -->|是| K
    ...
    style K0 fill:#e8f5e9,stroke:#4caf50
...
- 新增 BLEU Calculator 和 LLM Judge Calculator
...
flowchart LR
    ...
    A1 --> B1["config = {<br/>rounds: [{<br/>  audios: [{audio_id}],<br/>  algorithmParams: [{field_code, inputText/inputAudio}],<br/>  evaluation,<br/>  referenceParamsPath<br/>}],<br/>dimensions: [WER,BLEU,LLM Judge]<br/>}"]
    A2 --> B2["config = {<br/>rounds: [{<br/>  audios: [{audio_id,playbackDeviceId,spl}],<br/>  backgroundNoise: {audioId,deviceIds,spl,loop},<br/>  algorithmParams: [{field_code, interferers/railDistance/volumeLevel}],<br/>  evaluation,<br/>  referenceParamsPath<br/>}],<br/>dimensions: [WER,BLEU,LLM Judge]<br/>}"]
...
| `utils/api_executor.py` | 重构 | 多轮会话（配置驱动，rounds 非空时） |
...
| **新增** `app/services/bleu_calculator.py` | 新增 | BLEU 计算 |
