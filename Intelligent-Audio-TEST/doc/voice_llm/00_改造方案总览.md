...
### 背景
现有智能音频测试平台支持 translation（翻译）、asr（语音识别）、speaker_recognition（声纹识别）、tts（语音合成）等算法类型的自动化测试。voice_llm（语音交互大模型）作为新的测试类型，需要支持多轮对话、声纹注册、干扰人播放、导轨距离控制、被测设备音量控制等能力。
...
flowchart TD
    ...
    K --> K1["env_devices setup<br/>(导轨等环境设备)"]
    K1 --> K2["构建干扰人配置"]
    K2 --> K3["播放音频<br/>pre_process → 提示音 → pre_process → 混音播放<br/>干声+噪声+干扰人统一混音<br/>(音量由驱动 pre_process 自行管理)"]
    K3 --> K4["post_process<br/>(驱动自行恢复音量等)"]
    K4 --> K5["等待响应"]
    K5 --> K6["收集本轮结果"]
    K6 --> K7["标记 round_number"]
    K7 --> K8["env_devices teardown<br/>(导轨复位等)"]
    K8 --> K9{还有下一轮?}
    K9 -->|是| K
    K9 -->|否| W[汇总多轮结果]
    W --> X[入队整体评估]
    ...
    style K4 fill:#e8f5e9,stroke:#4caf50
...
- 聚合策略配置化：`Dimension.statistic_method` 字段控制聚合方式（average/weighted_wer），策略模式实现
- 输出字段配置化：`EvaluationDimensionParam` 表新增 param_direction/field_path/agg_role/output_role/visible_in_report
- ParamMapping 自动同步：评估维度保存时自动同步 input/output 字段映射
- 报告过滤：`visible_in_report=False` 的维度不显示在报告列中
...
```mermaid
flowchart LR
    A1["TestCase #1<br/>test_type='api'"] --> A2["TestCase #2<br/>test_type='e2e'"]
...
- After：API 单条记录（test_type='api'）+ E2E 记录（test_type='e2e'），config 扁平化为 `{rounds:[], dimensions:[]}`
...
| 文件 | 改动类型 | 说明 |
|------|---------|------|
| `models/models.py` | 修改 | TestCase 新增 test_type |
| `models/algorithm_models.py` | 修改 | CaseAlgorithmParam 新增 scope；EvaluationDimensionParam 新增 param_direction/field_path/agg_role/output_role/visible_in_report |
...
| `controllers/evaluation_controller.py` | 修改 | output 字段保存含 output_role/visible_in_report；ParamMapping 自动同步（input+output） |
...
| `utils/evaluation_result_processor.py` | 修改 | 多轮结果聚合；parse_dimension_result 改为配置驱动（output_role=main + field_path） |
| **新增** `utils/report/aggregation_strategies.py` | 新增 | 聚合策略模式（SimpleAverageStrategy / WeightedSumRatioStrategy） |
| `utils/report/report_utils.py` | 修改 | 维度聚合按 statistic_method 分配策略；加载 output_role/visible_in_report |
| `controllers/report_controller.py` | 修改 | 报告过滤 visible_in_report=False 的维度 |
| `controllers/report_controller_secondary.py` | 修改 | 报告过滤 visible_in_report=False 的维度 |
| `controllers/report_controller_compare.py` | 修改 | 报告过滤 visible_in_report=False 的维度 |
...
| `device_driver/base_driver.py` | 修改 | 新增 set_volume/get_volume/move_rail/reset_rail |
| `device_driver/harmony_driver.py` | 修改 | 音量/导轨具体实现 |
...
| `components/common/test-case/TestCaseListContainer.vue` | 修改 | test_type 列+筛选 + **视图切换器(分组/标签)** + 标签视图展示 (2026-07-06) |
...
| `store/testCaseStore.ts` | 修改 | test_type 状态管理 + **tagViewData/fetchTagView** (2026-07-06) |
...
| `components/common/modal/DetailViewModal.vue` | 修改 | **标注任意字段动态列 + data 顶层字段编辑** (2026-07-06) |
| `views/TestCaseManagerLogic/testCaseManager.ts` | 修改 | **viewMode 切换 + refreshCurrentView** (2026-07-06) |
| `views/TestCaseManager.vue` | 修改 | **传递 tagViewData 和 v-model:viewMode** (2026-07-06) |
| `views/AudioImportLogic/audioImport.ts` | 修改 | **集成 folderParser，构建 testCaseConfig 并传入 mergeChunks** (2026-07-06) |
| **新增** `utils/folderParser.ts` | **新增** | **文件夹解析工具：extractAudioFiles/buildRoundsConfig/buildTestCaseConfig** (2026-07-06) |
| **新增** `components/common/form/OutputFieldsEditor.vue` | 新增 | 评估维度输出字段配置编辑器（field_path/output_role/agg_role/visible_in_report） |
| ~~**新增** `components/common/test-case/TestCaseModal/SessionConfigEditor.vue`~~ | ~~新增~~ | ~~会话配置编辑器~~ （已移除，通过 DynamicForm 替代） |
