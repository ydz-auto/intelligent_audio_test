...
from audio_service.infrastructure.audio.audio_engine import audio_service as _default_audio_service
...
class PlaybackOrchestrator:
    ...
    def start_background_noise(self, case_config, task_id):
        ...
        仅当 case_config.background_noise 有效（含音频和设备）时启动。
    ...
    def _get_background_noise_device_indices(self, task_id):
        ...
        for k, v in players.items():
            if k.startswith(self._bg_noise_player_type):
                # player_key 格式: global_noise_<device_index>
                parts = k.rsplit('_', 1)
                if len(parts) == 2:
                    try:
    ...
    def play_voiceprint(self, vp_config, task_id):
        ...
        from audio_service.infrastructure.audio.playback_config_builder import _get_playback_device_via_grpc
        ...
        try:
            device_unique_id = dev_obj.get('device_unique_id')
            channel_index = dev_obj.get('channel_index', 0)
            spl_mapping_id = dev_obj.get('current_spl_mapping_id')
            device_index = (
