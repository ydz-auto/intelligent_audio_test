...
const PARAM_CODE_PRESETS: Record<string, {param_name: string; param_type: string; default_value?: string; help_text?: string; min_value?: number; max_value?: number; step?: number; unit?: string}> = {
  ...
  'voiceprint': { param_name: '声纹注册', param_type: 'audio_select', help_text: '声纹注册音频配置（含音频、设备、声压级、等待时间）' },
  'interferers': { param_name: '干扰人列表', param_type: 'audio_select', default_value: '[]', help_text: '干扰人配置列表（每个含音频、设备、声压级）' },
  ...
  'played_audios': { param_name: '被播放音频', param_type: 'audio_select', help_text: '本轮被播放音频（存于用例配置 rounds[].audios），评估时经 case_config→audios 映射取用' },
  'background_noise': { param_name: '背景噪声', param_type: 'audio_select', help_text: '背景噪声配置（用例级 config.background_noise 全局，或轮次级 rounds[].background_noise），评估时经 case_config→background_noise 映射取用' },
}
...
const FEATURE_BUNDLES: Record<string, { label: string; scope: string; params: string[] }> = {
  ...
  voiceprint: { label: '声纹注册', scope: 'e2e', params: ['voiceprint'] },
  ...
  case_config: { label: '用例配置(被播放音频/背景噪声/干扰人)', scope: 'e2e', params: ['played_audios', 'background_noise', 'interferers'] },
}
