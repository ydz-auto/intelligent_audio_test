...
export const formatAudioData = (audio: any): AudioData => {
  const filepath = audio.path || audio.filepath || audio.file_path || '';
  return {
    id: audio.id,
...
export function normalizeAudioItem(audio: any, fallbackTestType?: string): any {
  ...
  return {
    ...
    spl: audio.spl,
    testType: audioType,
    playOrder: audio.play_order,
    noiseSpl: audio.noise_spl,
    deviceId: audio.device_id,
    deviceName: audio.device_name,
    playbackDeviceName: audio.playback_device_name ?? audio.device_name,
    timelineStart,
    timelineEnd: audio.timeline_end ?? (timelineStart + (audio.duration || 0)),
    roundNumber: audio.round_number ?? audio.round ?? 1,
  };
}
