"""
报告管理模型 (Report Management)

包含测试报告及其摘要、元数据、原始数据、用例、指标统计、对比矩阵等子模型。
"""
from ._base import (
...
class Report(db.Model):
    ...
    task = relationship('Task', foreign_keys='Report.task_id', backref='reports')
...
class ReportSummary(db.Model):
    ...
    report_id = Column(BigInteger, nullable=False, unique=True, comment='关联报告ID')
