...
使用方法：
    ...
    python scripts/migrations/202609/seed_xiaoyi_dimensions.py
    ...
    },
    {
        ...
        'params': [
            ...
            ('false_takeover', '误接管率', '误接管率', 'number', 'output',
             ...
             False, None, '误接管率(0=未抢话, 1=抢话)', 70, 1),
            ...
            ('ft_llm_false_takeover', '误接管LLM判定', 'LLM判定是否误接管', 'number', 'output',
             'false_takeover.llm_eval.false_takeover', None, 'aux', True,
             False, None, 'LLM判定: 1=误接管, 0=正常', 76),
            ('ft_llm_reason', '误接管LLM判定理由', 'LLM判定理由', 'text', 'output',
             'false_takeover.llm_eval.reason', None, 'aux', True,
             False, None, 'LLM判定详细理由', 77),
    ...
    },
    {
        ...
        'params': [
            ...
            ('tl_user_last_word_end_ms', '用户末词结束时刻', '用户末词结束时刻', 'timestamp', 'output',
             'takeover_latency.user_last_word_end_ms', None, 'aux', True,
             False, None, 'takeover_latency: user 末词结束时间(毫秒)', 81),
            ('tl_ai_first_word_start_ms', '模型首词开始时刻', '模型首词开始时刻', 'timestamp', 'output',
             'takeover_latency.ai_first_word_start_ms', None, 'aux', True,
             False, None, 'takeover_latency: AI 首词开始时间(毫秒)', 82),
            ('tl_ai_skipped_opening_chunks', '跳过开场白段数', '跳过开场白段数', 'number', 'output',
             'takeover_latency.ai_skipped_opening_chunks', None, 'aux', True,
             False, None, 'takeover_latency: 跳过的开场白 chunk 数', 83),
            ('tl_ai_total_chunks', 'AI总段数', 'AI总段数', 'number', 'output',
             'takeover_latency.ai_total_chunks', None, 'aux', True,
             False, None, 'takeover_latency: ai_chunks 总数', 84),
    ...
    # ────────────────────────────────────────────────────────────
    {
        ...
        'params': [
            ...
            ('avg_response_latency_s', '平均回复时延(秒)', '平均回复时延(秒)', 'number', 'output',
             'high_freq_turn_taking.avg_response_latency_s', None, 'aux', True,
             False, None, '平均回复时延(秒)', 91),
            ('min_response_latency_s', '最小回复时延', '最小回复时延', 'number', 'output',
             'high_freq_turn_taking.min_response_latency_s', None, 'aux', True,
             False, None, '最小回复时延(秒)', 92),
            ('max_response_latency_s', '最大回复时延', '最大回复时延', 'number', 'output',
             'high_freq_turn_taking.max_response_latency_s', None, 'aux', True,
             False, None, '最大回复时延(秒)', 93),
            ('n_rounds', '总轮数', '总轮数', 'number', 'output',
             'high_freq_turn_taking.n_rounds', None, 'aux', True,
             False, None, '用户段总数(=轮数)', 94),
            ('n_matched_rounds', '匹配轮数', '匹配轮数', 'number', 'output',
             'high_freq_turn_taking.n_matched_rounds', None, 'aux', True,
             False, None, '成功匹配到AI回复的轮数', 95),
            ('n_missed_rounds', '未匹配轮数', '未匹配轮数', 'number', 'output',
             'high_freq_turn_taking.n_missed_rounds', None, 'aux', True,
             False, None, '未匹配到AI回复的轮数', 96),
            ('n_unmatched_ai_segments', '未消费AI段', '未消费AI段', 'number', 'output',
             'high_freq_turn_taking.n_unmatched_ai_segments', None, 'aux', True,
             False, None, '未被消费的AI段数(开场白/结束语等)', 97),
            ('n_user_segments', '用户段数', '用户段数', 'number', 'output',
             'high_freq_turn_taking.n_user_segments', None, 'aux', True,
             False, None, '用户段总数', 98),
            ('n_ai_segments', 'AI段数', 'AI段数', 'number', 'output',
             'high_freq_turn_taking.n_ai_segments', None, 'aux', True,
             False, None, 'AI段总数', 99),
            ('hftt_per_round', '每轮明细', '每轮明细', 'json', 'output',
             'high_freq_turn_taking.per_round', None, 'aux', True,
             False, None, '每轮匹配结果(轮次/用户段/AI段/时延)', 100),
    ...
    # ────────────────────────────────────────────────────────────
    {
        ...
        'params': [
            ...
            ('overall_pass_rate', '通过率', '通过率', 'number', 'output',
             ...
             False, None, '通过轮数/总轮数(0.0-1.0)', 110),
            ('hflj_enabled', '是否启用', '是否启用LLM裁判', 'boolean', 'output',
             'high_freq_llm_judge.enabled', None, 'aux', True,
             False, None, '是否启用 LLM 裁判', 111),
            ('hflj_model', 'LLM模型', 'LLM模型名称', 'text', 'output',
             'high_freq_llm_judge.model', None, 'aux', True,
             False, None, 'LLM 模型名', 112),
            ('hflj_scenario_type', '场景类型', '场景类型', 'text', 'output',
             'high_freq_llm_judge.scenario_type', None, 'aux', True,
             False, None, '场景类型(飞花令/成语接龙/快问快答/自定义)', 113),
            ('n_passed', '通过轮数', '通过轮数', 'number', 'output',
             'high_freq_llm_judge.n_passed', None, 'aux', True,
             False, None, '符合预期的轮数', 114),
            ('n_failed', '未通过轮数', '未通过轮数', 'number', 'output',
             'high_freq_llm_judge.n_failed', None, 'aux', True,
             False, None, '不符合预期的轮数', 115),
            ('hflj_n_rounds', '总轮数', '总轮数', 'number', 'output',
             'high_freq_llm_judge.n_rounds', None, 'aux', True,
             False, None, '评估的总轮数', 116),
            ('hflj_per_round', '每轮裁判', '每轮裁判', 'json', 'output',
             'high_freq_llm_judge.per_round', None, 'aux', True,
             False, None, '每轮 pass/fail + reason', 117),
            ('hflj_summary', '总结', '总结', 'text', 'output',
             'high_freq_llm_judge.summary', None, 'aux', True,
             False, None, '自然语言总结', 118),
            ('hflj_tokens_used', 'Token总数', 'Token总数', 'number', 'output',
             'high_freq_llm_judge.tokens_used', None, 'aux', True,
             False, None, '总 token 数', 119),
            ('hflj_input_token', '输入Token', '输入Token', 'number', 'output',
             'high_freq_llm_judge.input_token', None, 'aux', True,
             False, None, '输入 token 数', 120),
            ('hflj_output_token', '输出Token', '输出Token', 'number', 'output',
             'high_freq_llm_judge.output_token', None, 'aux', True,
             False, None, '输出 token 数', 121),
