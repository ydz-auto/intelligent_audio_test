...
背景：
   ...
   source=case_config 取这些字段，注入到评估维度 input 参数。
...
用途：
   ...
   - evaluation_service 在评估映射含 case_config source 时加载 test_case.config，
     经 _build_rounds_list / _build_single_round 按轮映射为对应参数，
     音频路径运行时经 _normalize_round_eval_fields 补全并 multipart 上传、
     算法侧 eval_server 落盘还原

对应实现：
   - evaluation_service/domain/services/evaluation_service/round_data_builder.py
     case_config 分支 + _normalize_round_eval_fields + _fill_round_audio_paths
   - evaluation_service/infrastructure/evaluation_api/api_request_handler.py
     _extract_nested_files（递归提取 multipart 上传，替换 __MULTIPART__ 占位符）
   - eval_server（算法侧服务，仓库外）           占位符还原为落盘路径

使用方法：
    cd Intelligent-Audio-TEST
    python scripts/migrations/202609/seed_stimulus_audios.py
...
def _upsert_param(conn, dim_id, param_def):
    ...
    param_def 顺序与 AUDIO_PARAMS 注释一致。
    """
    (param_code, param_name, label, field_type, param_direction,
     field_path, agg_role, output_role, visible_in_report,
...
if __name__ == '__main__':
    ...
    print("执行链路：前端映射 case_config → 用例配置 rounds[].{audios,background_noise,"
          "interferers}")
    print("→ evaluation_service 按轮归一化 case_config（audio_path 运行时补全）")
    print("→ api_request_handler multipart 上传 → 算法侧 eval_server 占位符还原为本地文件")
    print()
    print("脚本可重复执行（幂等）")
    print()
