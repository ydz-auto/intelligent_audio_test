...
from shared.models.models import Dimension, TestResultDimension, TaskCase, TestResult, Task, TaskDevice, TaskAPI, TestCase, utc8now
from shared.models.database import db
from task_service.evaluation.evaluation_utils import extract_by_path, calculate_score
...
class EvaluationResultProcessor(RoundAggregator):
    ...
    def update_task_case_status(self, result_id, current_result_all_completed, task_id, test_case_id, test_type=None):
        ...
        try:
            ...
            try:
                ...
                # 2. 获取预期结果数量
                expected_count = self._get_expected_result_count(task_id, task, local_db_session)

                # 3. 获取该用例目前已生成的所有测试结果
                all_results = local_db_session.query(TestResult).filter_by(task_id=task_id, test_case_id=test_case_id).all()

                # 4. 获取用例配置中预期的评估维度总数
                expected_dim_count = self._get_expected_dim_count(test_case_id, local_db_session)

                # 5. 检查是否所有预期结果都已采集并完成评估
                case_all_finished, case_any_failed = self._check_all_results_completed(
                    task_id, test_case_id, expected_count, expected_dim_count, all_results, local_db_session
                )
                ...
                self._apply_final_status(task_id, test_case_id, new_status, new_evaluation_status, local_db_session)
            except Exception as e:
                self._log(
                    level='ERROR',
                    category='database',
    ...
    def _get_expected_result_count(self, task_id, task, local_db_session):
        """获取预期结果数量"""
        expected_count = 0
        if task.type == 'e2e':
    ...
    def _get_expected_dim_count(self, test_case_id, local_db_session):
        """获取预期维度数量（从config解析）"""
    ...
    def _check_all_results_completed(self, task_id, test_case_id, expected_count, expected_dim_count, all_results, local_db_session):
    ...
    def _apply_final_status(self, task_id, test_case_id, new_status, new_evaluation_status, local_db_session):
        """应用最终状态"""
    ...
    def process_group_dimension_results(self, resp_data, group_items, task_id, test_case_id, result_id, api_request_body, test_type='api'):
        ...
        try:
            ...
            device_id = None
            if result_id:
                test_result = local_db_session.get(TestResult, result_id)
                if test_result:
