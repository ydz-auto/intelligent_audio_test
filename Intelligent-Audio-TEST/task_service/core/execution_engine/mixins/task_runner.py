...
的导入路径（TaskRunnerMixin 为组合类）：

- task_lifecycle.py：TaskLifecycleMixin — 任务启动 / API 初始化 / 主循环编排
- task_dispatch.py：TaskDispatchMixin — 用例领取 / 原子占用 / 分发 / 失败处理
- device_check.py：DeviceCheckMixin — E2E 被测设备 / 播放设备在线检查
- task_finalize.py：TaskFinalizeMixin — 等待完成 / 状态收敛 / 异常 / 资源清理
"""
