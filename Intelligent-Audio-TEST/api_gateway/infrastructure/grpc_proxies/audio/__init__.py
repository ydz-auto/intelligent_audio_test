"""音频服务代理子包：统一 re-export 全部音频相关代理类与模块级单例。

原 1117 行大文件已按域拆分至以下子模块，本包作为门面保持对外导出不变：
- service_proxy.py               — audio_service（播放/停止/设备/播放器快照）及别名 AudioService
...
- config_proxy.py                — audio_config_service（音频元数据/标注/上传/转换/预览，组合 mixin）
