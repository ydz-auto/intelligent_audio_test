...
将 algorithm_controller 中的写操作函数迁移为 AlgorithmCommandService 的静态方法。
保留原有逻辑，不改业务。
"""
...
from .algorithm_common import _serialize_algorithm
from .algorithm_query_service import AlgorithmQueryService
...
class AlgorithmCommandService:
    ...
    def create_algorithm():
        ...
        if req.device_params is not None:
            AlgorithmCommandService._update_params(req.type, req.device_params, 'device')
        if req.api_params is not None:
            AlgorithmCommandService._update_params(req.type, req.api_params, 'api')
        if req.case_params is not None:
            AlgorithmCommandService._update_case_params(req.type, req.case_params)
        if req.mappings is not None:
            AlgorithmCommandService._update_mappings(req.type, req.mappings)
        if req.associated_dimensions is not None:
            AlgorithmCommandService._update_associated_dimensions(req.type, req.associated_dimensions)
    ...
    def update_algorithm(algo_type: str):
        ...
        if req_data.device_params is not None:
            AlgorithmCommandService._update_params(algo_type, req_data.device_params, 'device')
        if req_data.api_params is not None:
            AlgorithmCommandService._update_params(algo_type, req_data.api_params, 'api')
        if req_data.case_params is not None:
            AlgorithmCommandService._update_case_params(algo_type, req_data.case_params)
        if req_data.mappings is not None:
            AlgorithmCommandService._update_mappings(algo_type, req_data.mappings)
        if req_data.associated_dimensions is not None:
            AlgorithmCommandService._update_associated_dimensions(algo_type, req_data.associated_dimensions)
    ...
    def create_param():
        ...
        from .algorithm_common import _serialize_api_param, _serialize_device_param
        if param_type_source == 'api':
