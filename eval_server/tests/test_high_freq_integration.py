...
def test_import():
    """1. 验证模块导入正常"""
    from app.services.xiaoyi_metrics.turn_taking import (
...
def test_compute_high_freq_turn_taking():
    ...
    from app.services.xiaoyi_metrics.turn_taking import compute_high_freq_turn_taking
...
def test_calculate_xiaoyi_metrics_integration():
    ...
    import app.services.xiaoyi_metrics.turn_taking as tt_module
    from app.services.xiaoyi_metrics.turn_taking import calculate_xiaoyi_metrics
...
def test_task_service_no_high_freq_branch():
    ...
    assert "'turn_taking'" in source, "task_service 应包含 turn_taking 分支"

    print("[PASS] task_service.py 已移除独立 high_freq 分支，统一由 turn_taking 处理")
    return True
