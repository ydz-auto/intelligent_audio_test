...
设计一个有评分梯度的 5 轮打断对话，验证 LLM 能否打出区分度：
  R1 打断(优秀回复)       → 期望高分 5
  R2 打断(中等:简短无衔接) → 期望中分 3-4
  R3 打断(答非所问/兜底)   → 期望低分 1-2
  R4 回原话题(成功恢复)    → 行为=回应/恢复, 高分
  R5 回原话题(简短/形同沉默)→ 行为=沉默/无关恢复, 低分
...
ROUNDS = [
    ...
    },
    {
        ...
        'is_return_to_topic': False,
        '_expect': 'R3 打断·低分(答非所问+兜底话术，未满足用户需求) → 期望 1-2',
    },
    {
        'query': '我们还是回到刚才说的电影吧',
    ...
    },
    {
        ...
        'answer': '嗯，好的。',
        'is_return_to_topic': True,
        '_expect': 'R5 回原话题·失败(简短形同沉默，未真正回到原话题内容) → 行为=沉默/无关恢复, 低分',
    },
]
