...
| 参数               | 类型    | 必填 | 默认值   | 说明                                           |
| ------------------ | ------- | -- | ------- | -------------------------------------------- |
| `ai_wav`           | str     | 是 | `''`    | 模型回复音频路径（主输入，LLM 直接听音频判断行为）                |
| `user_wav`         | str     | 否 | `''`    | 用户通道音频路径（直接发给 LLM + 经 ASR 转写生成时间线上下文）        |
| `model`            | str     | 否 | `''`    | LLM 模型名，缺省读 `config.LLM_JUDGE.default_model` |
| `max_tokens`       | int     | 否 | `4096`  | 最大输出 token                                   |
| `temperature`      | float   | 否 | `0.1`   | 采样温度                                        |
| `is_single_round`  | bool    | 否 | `False` | `True`=单轮拒识（user_wav 直接为拒识内容），`False`=多轮拒识（user_wav 包含意图交互+拒识干扰） |
| `timing`           | str     | 否 | `''`    | 拒识发生时机，手动传入（`"回复过程中"` / `"静默"`），决定行为类别定义和 rate 计算 |
...
```json
{
  ...
  "evaluations": [
    {
      "behavior": "回应",             // 回应/恢复/不确定询问/无关回复/静默
      "reason": "模型在干扰内容出现后回复了相关内容..."
    }
  ],
  "interaction_text": "query [0:05; 0:09]...\nanswer [0:12; 0:13]...\n",
  "query": "用户侧ASR文本",
  "answer": "模型侧ASR文本",
  "behavior_respond": 1,            // 回应 → 1, 否则 0
...
| 场景 | 名称 | 说明 |
| ---- | ---- | ---- |
| 1 | 非目标人拒识 | 在目标人与模型对话过程中，出现的非目标人干扰讲话 |
| 2 | 目标人非交互意图 | 在目标人与模型对话过程中，出现了目标人临时插入与其他人的讲话、打电话等，不具备与模型交互意图的语音 |
| 3 | 环境噪声 | 音频中出现环境噪声，如关门声、交通噪声、音乐声等非语音噪声 |
| 4 | 用户BC | 用户在交互过程中发出"嗯""啊""好的""对"等短反馈词，不具备发起新请求的意图 |

### 行为类别定义（5种，按 timing 分两套）

**timing=回复过程中**：回应 / 恢复 / 不确定询问 / 无关回复 / 静默

**timing=静默**：回应 / 恢复 / 不确定询问 / 无关回复 / 静默（定义不同）
