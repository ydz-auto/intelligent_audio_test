...
│   ├── strategy.py                  #   TurnTakingCalculator + InterruptionMetricsCalculator 策略类
...
│   ├── __init__.py
│   ├── strategy.py                  #   EnvJudgeCalculator 策略类
...
calculators/__init__.py              # import 即自动注册全部 calculator
  ...
  └── xiaoyi_metrics/                #   本包
      ├── turn_taking/strategy.py     #     TurnTaking + InterruptionMetrics
      ├── rejection_scene_awareness/strategy.py  # NonInteractiveLatency + NoiseLatency
      ├── env_judge/strategy.py       #     EnvJudge
...
| 函数 | 用途 | 对应 task_type |
|------|------|----------------|
| `calculate_xiaoyi_metrics(task_params)` | 调一次 ASR，六维度共享结果（tor / false_takeover / takeover_latency / input_asr / interruption / non_interactive_latency） | `turn_taking` |
...
##### 5. interruption —— 打断指标 [interruption.py](interruptbility/interruption.py)
...
##### 6. interruption_llm —— 打断 LLM 评估 [interruption_llm.py](interruptbility/interruption_llm.py)
...
##### 7. non_interactive_latency —— 非交互意图时延 [non_interactive_latency.py](rejection_scene_awareness/non_interactive_latency.py)
...
##### 8. noise_latency —— 噪声打断时延 [noise_latency.py](rejection_scene_awareness/noise_latency.py)
...
### env_judge —— 环境音/打断能力录屏裁判 [env_judge/env_judge.py](env_judge/env_judge.py)
...
TaskService.CALCULATORS（注册表）
  ├── 'turn_taking'              → TurnTakingCalculator → calculate_xiaoyi_metrics
  ...
  ├── 'env_judge'                → EnvJudgeCalculator → evaluate_env_judge
